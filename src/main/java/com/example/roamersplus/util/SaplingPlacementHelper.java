...
public class SaplingPlacementHelper {
     ...
     * @param saplingStacks List of sapling stacks to scatter (will be modified with remainders)
     */
    public static void scatterSaplingsAlternating(Level level, BlockPos centerPos, int radius, List<ItemStack> saplingStacks) {
        if (level.isClientSide() || saplingStacks.isEmpty()) {
            return;
        }
        
        // Filter to only valid sapling stacks
        List<ItemStack> validStacks = new ArrayList<>();
        List<Block> saplingBlocks = new ArrayList<>();
        ...
        // Alternate through sapling types
        int saplingTypeIndex = 0;
        
        for (BlockPos basePos : potentialPositions) {
            // Find the next sapling type that still has saplings to place
            int startIndex = saplingTypeIndex;
            ItemStack currentStack = null;
            Block currentBlock = null;
            
            do {
                currentStack = validStacks.get(saplingTypeIndex);
                currentBlock = saplingBlocks.get(saplingTypeIndex);
                saplingTypeIndex = (saplingTypeIndex + 1) % validStacks.size();
                
                if (!currentStack.isEmpty()) {
                    break;
                }
            } while (saplingTypeIndex != startIndex);
            ...
            // Find valid position and place
            BlockPos validPos = findValidSaplingPosition(level, basePos);
            if (validPos != null) {
                BlockState saplingState = currentBlock.defaultBlockState();
                level.setBlock(validPos, saplingState, 3);
                currentStack.shrink(1);
                
                RoamersPlusMod.LOGGER.debug("Scattered {} at {}", currentBlock.getName().getString(), validPos);
            }
        }
    }
