...
public class SaplingPlacementHelper {
    ...
    public static void scatterSaplingsAlternating(Level level, BlockPos centerPos, int radius, 
        ...
        // Build parallel lists of valid stacks and their blocks
        ...
        RoamersPlusMod.LOGGER.debug("Scattering {} types of saplings around {}", validStacks.size(), centerPos);
        ...
        for (BlockPos basePos : potentialPositions) {
            ...
            int attempts = 0;
            while (attempts < validStacks.size()) {
                ItemStack currentStack = validStacks.get(typeIndex);
                if (!currentStack.isEmpty() && currentStack.getCount() > 0) {
                    ...
                    if (validPos != null) {
                        ...
                        typeIndex = (typeIndex + 1) % validStacks.size();
                        break;
                    }
                }
                // Try next type
